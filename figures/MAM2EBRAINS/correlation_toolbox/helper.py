...
def create_gamma_spiketrains(rate, T, N, k):
    ...
    size = int(1.5 * rate * T)
    times = np.hstack(
        [np.cumsum([np.random.gamma(rate, k, size=size)]) for i in range(N)]
    )
...
def sort_gdf_by_id(data, idmin=None, idmax=None):
    ...
    assert (idmin is None and idmax is None) or (
        idmin is not None and idmax is not None
    )
...
def gdf_to_neo(
    data, quantity, block, index, idmin=None, idmax=None, t_start=None, t_stop=None
):
    """
...
def create_correlated_spiketrains_sip(rate, T, N, cc):
    ...
    returns gdf: [(id,time),...]
    """
    rated = (1.0 - cc) * rate  # rate of independent (disjoint) processes
...
def movav(y, Dx, dx):
    ...
    if Dx <= dx:
        ...
        r[0 : np.int(n / 2.0)] = 1.0 / n
        r[-np.int(n / 2.0) : :] = 1.0 / n
        R = np.fft.fft(r)
        Y = np.fft.fft(y)
        yf = np.fft.ifft(Y * R)
        return yf
